...
flowchart LR
    ...
    D --> T["技术视角投影<br/>接口 · 模块 · 依赖收窄"]
    T --> E["影响评估报告<br/>业务+技术双视角 / 建议 / 回归范围"]
...
- **技术视角影响面**：业务影响面投影到接口/模块（direct → 需改动，下游 → 回归验证），
  并按接口级调用关系把依赖影响从「整个前端」收窄到「结算页模块」；
  接口图谱（interfaces.yaml）可选、可按域渐进补充，缺失部分自动降级为系统级结论。
  → [docs/04-技术视角影响面.md](docs/04-技术视角影响面.md)
...
（需求概要 / 用户旅程 / 故事→功能点映射 / 四层影响面 + 影响图 + 技术影响面（接口/模块）/
泳道图 / 建议 / 假设与待澄清）。
...
data/panorama/          业务全景图（图谱即代码，示例为电商域）
  ...
  interfaces.yaml       接口/模块层（可选，技术视角：契约、调用方、功能点映射）
src/req_insight/
...
- [ ] 接口清单导入器：从 OpenAPI / proto / 服务注册中心同步生成 interfaces.yaml
      的接口部分，人只维护功能点 ↔ 接口映射（技术视角的目标态）
